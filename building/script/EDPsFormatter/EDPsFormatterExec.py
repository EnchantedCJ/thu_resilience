# -*- coding: utf-8 -*-
from .EDPsFormatter import idr2comp


def main(dirIn, dirOut, dirAttr, config):
    numEQ = config['numEQ']
    ...
    blg = []  # 输出文件每栋建筑的记录
    edp = []  # 输入文件每行的记录
    with open(dirIn, 'r', encoding='utf-8') as f:
        i = 0
    ...
    blgAtts = []
    with open(dirAttr, 'r', encoding='utf-8') as f:
    ...
    f = open(dirOut, 'w', encoding='utf-8')
    f.writelines('{nBlg}\n'.format(nBlg=numBlg))
    for i in range(numBlg):
        f.writelines(blg[i])
    f.close()
...
if __name__ == '__main__':
    config = {'numEQ': 1,
              'idr2comp': True}
    dirIn = '../tha/Results/EDPs.txt'
    dirOut = './LossEstimator/input/Edps.txt'
    dirAttr = '../tha/inputs/BlgAttributes.txt'
    main(dirIn, dirOut, dirAttr, config)
